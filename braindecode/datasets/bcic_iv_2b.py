...
class BCICompetition4Set2B(object):
    name_to_code_train = {'1023': 1, '1077': 2, '1078': 3, '1079': 4, '1081': 5, '276': 6, '277': 7, '32766': 8,
                          '768': 9, '769': 10, '770': 11, '781': 12}
    name_to_code_test = {'1023': 1, '1077': 2, '1078': 3, '1079': 4, '1081': 5, '276': 6, '277': 7, '32766': 8,
                         '768': 9, '783': 10, '781': 12}
    ...
    def extract_events(self, raw_gdf):
        ...
        if not ("783" in name_to_code):
            train_set = True
            assert all([s in name_to_code for s in ["769", "770"]])
        else:
            train_set = False
            assert ("783" in name_to_code)

        if train_set:
            trial_codes = [10, 11]  # the 2 classes
        else:
            trial_codes = [10]  # "unknown" class

        trial_mask = np.array(
            [ev_code in trial_codes for ev_code in events[:, 2]])
        trial_events = np.array(events[trial_mask]).copy()
        # assert len(trial_events) == 120 or len(trial_events) == 160, "Got {:d} markers".format(
        #     len(trial_events)
        # )
        print("len(trial_events)={}".format(len(trial_events)))
